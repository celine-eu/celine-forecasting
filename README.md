...
All infrastructure config (DB, MLflow, MinIO) is managed via pydantic-settings with dev defaults in `core/settings.py`. Override via `.env` file or environment variables. See `.env.example`.
