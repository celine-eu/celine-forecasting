    ...
    *,
    ...
    if incremental:
        ...
        if lookback is not None:
            ...
            df_train = df_train[df_train[COL_TS_HOUR] >= cutoff].reset_index(drop=True)
            logger.info("Incremental mode: training on last %d day(s) of data", lookback)
    ...
    with tracker.run(run_name=f"{device}"):
        ...
        for target in config.targets:
            ...
            models = train_band_models(
                ...
                has_pv=has_pv,
                available_columns=available_columns,
                previous_models=prev_target_models,
            )
            if models is None:
                continue
            trained[target] = models

            train_mae = _quick_train_mae(dev, target, models, config, available_columns)
        ...
        if trained:
            all_band_models = {}
            for target, target_models in trained.items():
                for band_name, bundle in target_models.items():
