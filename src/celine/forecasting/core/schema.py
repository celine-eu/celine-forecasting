...
# ---------------------------------------------------------------------------
# Domain schema: role-based column mapping, shared across forecasting domains
# ---------------------------------------------------------------------------


@dataclass(frozen=True)
class DomainSchema:
    """Role-based column mapping for a forecasting domain.

    A forecasting domain (e.g. meters, recs, substations) exposes one
    ``DomainSchema`` instance describing which columns play which role, so
    domain-agnostic code (pooling, experiment naming, ...) can be written
    once against these roles instead of domain-specific column names.

    Attributes:
        entity_column: Name of the column identifying the forecasted entity,
            e.g. ``"device_id"``, ``"rec_id"``, or ``"substation_id"``.
        timestamp_column: Name of the timestamp column, e.g. ``"ts_hour"`` or
            ``"datetime"``.
        targets: Names of the columns forecasted for this domain.
        frequency: Expected sampling frequency (pandas offset alias), e.g.
            ``"15min"`` or ``"1h"``.
    """
