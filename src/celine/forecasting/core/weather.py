...
:class:`celine.forecasting.schema.WeatherDataContract`). In the demonstrator these
...
def build_weather_features(
    ...
    *,
    ...
    The output matches :class:`celine.forecasting.schema.WeatherDataContract`
    (``datetime`` + the 12 recommended columns). ``ghi_ramp`` is intentionally
    *not* produced here — it is derived downstream by
    :func:`celine.forecasting.cleaning.prepare_weather`.
